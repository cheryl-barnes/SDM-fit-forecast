...
# Intermediate Dynamic GAMs, Walleye Pollock (WEP):
...
d2GAM.WEPpa.full = gam(PA ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(phi, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WEP, family = binomial(link="cloglog"), method = "GCV.Cp")
  summary(d2GAM.WEPpa.full)
  save(d2GAM.WEPpa.full, file = "Analyses/d2GAM_WEPpa_full.rda")
# load("Analyses/d2GAM_WEPpa_full.rda")
...
d2GAM.WEPpa.check.full = round(as.data.frame(concurvity(d2GAM.WEPpa.full, full=F)), digits=3); d2GAM.WEPpa.check.full
# bathy-BPI > 0.5, exclude BPI
# phi-lon/lat > 0.5, exclude phi
# phi-CPE > 0.5, exclude phi
d2GAM.WEPpa.ind.cov.full = gam(PA ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WEP, family = binomial(link="cloglog"), method = "GCV.Cp") 
  summary(d2GAM.WEPpa.ind.cov.full)
# R-sq = 0.459; Dev. = 44.5%; UBRE = -0.54678
  save(d2GAM.WEPpa.ind.cov.full, file = "Analyses/d2GAM_WEPpa_ind_cov.rda")
# load("Analyses/d2GAM_WEPpa_ind_cov.rda")
  
# Generate alternative models using backwards step-wise selection:
# Remove sponges (p > 0.1):
d2GAM.WEPpa.alt.1 = gam(PA ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + corals + whips + offset(log(AreaSwept)), data = WEP, family = binomial(link="cloglog"), method = "GCV.Cp") 
  summary(d2GAM.WEPpa.alt.1)
# R-sq = 0.459; Dev. = 44.5%; UBRE = -0.54689
  save(d2GAM.WEPpa.alt.1, file = "Analyses/d2GAM_WEPpa_alt1.rda")
# load("Analyses/d2GAM_WEPpa_alt1.rda")

# Remove whips (p > 0.1):
d2GAM.WEPpa.alt.2 = gam(PA ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + corals + offset(log(AreaSwept)), data = WEP, family = binomial(link="cloglog"), method = "GCV.Cp") 
  summary(d2GAM.WEPpa.alt.2)
# R-sq = 0.459; Dev. = 44.5%; UBRE = -0.54693
  save(d2GAM.WEPpa.alt.2, file = "Analyses/d2GAM_WEPpa_alt2.rda")
# load("Analyses/d2GAM_WEPpa_alt2.rda")

# Save best-fit model based on GCV/UBRE:  
d2GAM.WEPpa.best = d2GAM.WEPpa.alt.2 
  summary(d2GAM.WEPpa.best)
  save(d2GAM.WEPpa.best, file = "Analyses/d2GAM_WEPpa_best.rda")
# load("Analyses/d2GAM_WEPpa_best.rda")
...
# Abundance #
# Full model:
d2GAM.WEP.N.full = gam(adjN ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(phi, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WEP_catch, family = poisson(link="log"), method = "GCV.Cp")  
  summary(d2GAM.WEP.N.full)
  save(d2GAM.WEP.N.full, file = "Analyses/d2GAM_WEP_N_full.rda")
# load("Analyses/d2GAM_WEP_N_full.rda")

# Re-run the full model without covariates that exhibit high levels of concurvity:
  # 'estimate' values >= 0.5 * significant concurvity
d2GAM.WEP.N.check = round(as.data.frame(concurvity(d2GAM.WEP.N.full, full=F)), digits=3); d2GAM.WEP.N.check
# bathy-lon/lat > 0.5, exclude bathy
# phi-lon/lat, exclude phi
# phi-CPE > 0.5, exclude phi
d2GAM.WEP.N.ind.cov = gam(adjN ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WEP_catch, family = poisson(link="log"), method = "GCV.Cp")
  summary(d2GAM.WEP.N.ind.cov)
# adj-R2 = 0.117; Deviance = 29.8%; UBRE = 880.29
  save(d2GAM.WEP.N.ind.cov, file = "Analyses/d2GAM_WEP_N_ind_cov.rda")
# load("Analyses/d2GAM_WEP_N_ind_cov.rda")
  
# Save best-fit model: 
d2GAM.WEP.N.best = d2GAM.WEP.N.ind.cov
  summary(d2GAM.WEP.N.best); sum(d2GAM.WEP.N.best$edf)
  save(d2GAM.WEP.N.best, file = "Analyses/d2GAM_WEP_N_best.rda")
# load("Analyses/d2GAM_WEP_N_best.rda")
    
#########################################################################
# Biomass #
# Full model:
d2GAM.WEP.WT.full = gam(adjWT ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(phi, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WEP_catch, family = Gamma(link="log"), method = "GCV.Cp")  
  summary(d2GAM.WEP.WT.full)
  save(d2GAM.WEP.WT.full, file = "Analyses/d2GAM_WEP_WT_full.rda")
# load("Analyses/d2GAM_WEP_WT_full.rda")

# Re-run the full model without covariates that exhibit high levels of concurvity:
  # 'estimate' values >= 0.5 * significant concurvity
d2GAM.WEP.WT.check = round(as.data.frame(concurvity(d2GAM.WEP.WT.full, full=F)), digits=3); d2GAM.WEP.WT.check
# bathy-lon/lat, exclude bathy
# phi-lon/lat, exclude phi
# phi-CPE, exclude phi
d2GAM.WEP.WT.ind.cov = gam(adjWT ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WEP_catch, family = Gamma(link="log"), method = "GCV.Cp")
  summary(d2GAM.WEP.WT.ind.cov)
# adj-R2 = 0.0541; Deviance = 31.2%; GCV = 1.9345
  save(d2GAM.WEP.WT.ind.cov, file = "Analyses/d2GAM_WEP_WT_ind.cov.rda")
# load("Analyses/d2GAM_WEP_WT_ind_cov.rda")

# Save best-fit model: 
d2GAM.WEP.WT.best = d2GAM.WEP.WT.ind.cov
  summary(d2GAM.WEP.WT.best)
  save(d2GAM.WEP.WT.best, file = "Analyses/d2GAM_WEP_WT_best.rda")
# load("Analyses/d2GAM_WEP_WT_best.rda")
...
WEPpa.pred = predict.gam(d2GAM.WEPpa.best, newdata = dynamic.model.grid, 
                         type = "response", se.fit = T) 
WEPpa.predict = cbind(dynamic.model.grid,WEPpa.pred)
WEPpa.predict = WEPpa.predict %>%
  rename(pa.fit = fit, pa.se.fit = se.fit)
  save(WEPpa.predict, file = "Analyses/d2WEP_pa_predict.rda")
# load("Analyses/d2WEP_pa_predict.rda")

# Abundance:
WEP.N.pred = predict.gam(d2GAM.WEP.N.best, newdata = dynamic.model.grid, 
                         type = "response", se.fit = T)
WEP.N.predict = cbind(dynamic.model.grid,WEP.N.pred)
WEP.N.predict = WEP.N.predict %>%
  rename(N.fit = fit, N.se.fit = se.fit)
  save(WEP.N.predict, file = "Analyses/d2WEP_N_predict.rda")
# load("Analyses/d2WEP_N_predict.rda")

# Biomass:
WEP.WT.pred = predict.gam(d2GAM.WEP.WT.best, newdata = dynamic.model.grid, 
                          type = "response", se.fit = T)
WEP.WT.predict = cbind(dynamic.model.grid,WEP.WT.pred)
WEP.WT.predict = WEP.WT.predict %>%
  rename(WT.fit = fit, WT.se.fit = se.fit)
  save(WEP.WT.predict, file = "Analyses/d2WEP_WT_predict.rda")
# load("Analyses/d2WEP_WT_predict.rda")
...
WEP.predict$WEP_Bio = with(WEP.predict, pa.fit * WT.fit)
  save(WEP.predict, file = "Analyses/d2WEP_predict.rda")
# load("Analyses/d2WEP_predict.rda")  
    
#################################################################
# Quantify model performance (metrics: correlation coefficients, [relative] RMSE, reliability index, average error, average absolute error, modeling efficiency - Stow et al. 2009):
...
for(i in unique(WEP_sf$Year)) {
  ...
  d2WEP.obs.pred[[i]] = st_join(OBS, PRED, join = st_nearest_feature, left = T)}
d2WEP_full = as.data.frame(dplyr::bind_rows(d2WEP.obs.pred))
d2WEP_full = d2WEP_full %>%
  rename(Year = Year.x)
  save(d2WEP_full, file = "Analyses/d2WEP_obs_pred_yr.rda")
# load("Analyses/d2WEP_obs_pred_yr.rda")

# correlation coefficients (tendency to co-vary, range: -1 to 1):
# presence-absence:
pa.cor.S = cor.test(d2WEP_full$PA, d2WEP_full$pa.fit, method = c("spearman")); pa.cor.S
# S = 2.9178e+11; p < 0.001; rho = 0.5190723 

pa.cor.yr = NULL
for(i in unique(d2WEP_full$Year)) {
  cor.data = subset(d2WEP_full, Year == i)
...
# abundance:
N.cor.S = cor.test(d2WEP_full$adjN, d2WEP_full$WEP_Abun, method = c("spearman")); N.cor.S
# S = 1.8781e+11; p < 0.001; rho = 0.6904387 
...
# biomass:
WT.cor.S = cor.test(d2WEP_full$adjWT, d2WEP_full$WEP_Bio, method = c("spearman")); WT.cor.S
# S = 2.0747e+11; p < 0.001; rho = 0.6580341
...
pa.model = gam(PA ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(bathy, bs='tp', k=6, m=1) + s(slope, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + corals + offset(log(AreaSwept)), data = pa.data, family = binomial(link="cloglog"), method = "GCV.Cp") 
...
pa.predictions_all = pa.predictions_all %>%
  rename(pa.fit = fit, pa.se.fit = se.fit)
  save(pa.predictions_all, file = "Analyses/d2WEP_pa_retro.rda") 
# load("Analyses/d2WEP_pa_retro.rda")
...
N.model = gam(adjN ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = N.data, family = poisson(link="log"), method = "GCV.Cp")
...
N.predictions_all = N.predictions_all %>%
  rename(N.fit = fit, N.se.fit = se.fit)
  save(N.predictions_all, file = "Analyses/d2WEP_N_retro.rda") 
# load("Analyses/d2WEP_N_retro.rda")
...
WT.model = gam(adjWT ~ s(Year, bs="cr", m=1) + te(lon, lat, bs='tp', m=1) + s(slope, bs='tp', k=4, m=1) + s(BPI, bs='tp', k=4, m=1) + s(ROMS_BT, bs='tp', k=4, m=1) + s(lon, lat, by=CPE, bs='tp', m=1) + sponges + corals + whips + offset(log(AreaSwept)), data = WT.data, family = Gamma(link="log"), method = "GCV.Cp") 
...
WT.predictions_all = WT.predictions_all %>%
  rename(WT.fit = fit, WT.se.fit = se.fit)
  save(WT.predictions_all, file = "Analyses/d2WEP_WT_retro.rda") 
# load("Analyses/d2WEP_WT_retro.rda")

# all models:
pred.pos_all = N.predictions_all %>% full_join(WT.predictions_all)
  save(pred.pos_all, file = "Analyses/d2WEP_retro_pos.rda") 
# load("Analyses/d2WEP_retro_pos.rda")
...
pred_all$WEP_Bio.pers = with(pred_all, (pa.fit * fit_WT.pers))
  ...
  save(pred_all, file = "Analyses/d2WEP_retro.rda") 
# load("Analyses/d2WEP_retro.rda")
...
# Merge data from model fitting and predicting:
d2WEP.obs.pred_all = NULL
for(model in unique(pred_all_sf$fitted.through)) {
  d2WEP.pred = subset(pred_all_sf, fitted.through == model)
   for(yr in unique(d2WEP.pred$Year)) {
     d2WEP.pred.yr = subset(d2WEP.pred, Year == yr)
     d2WEP.obs.yr = subset(WEP_sf, Year == yr)
     d2WEP.obs.pred_yr = st_join(d2WEP.obs.yr, d2WEP.pred.yr, join = st_nearest_feature, left=T)
d2WEP.obs.pred_all = rbind(d2WEP.obs.pred_all, d2WEP.obs.pred_yr)} }

d2WEP_retro_all = as.data.frame(d2WEP.obs.pred_all)
  save(d2WEP_retro_all, file = "Analyses/d2WEP_obs_pred_retro.rda")
# load("Analyses/d2WEP_obs_pred_retro.rda")

# Estimate Spearman's correlation coefficient and RMSE:
d2WEP_retro_all$fitted.through = as.factor(d2WEP_retro_all$fitted.through)
  format.pval(c(0.1, 0.0001, 1e-27))

d2WEP_retro_all = d2WEP_retro_all %>%
  rename(Year = Year.x)

# presence-absence:
pa.obs.pred.cor_all = NULL
for(model in unique(d2WEP_retro_all$fitted.through)) {
  cor.pa.data = subset(d2WEP_retro_all, fitted.through == model)
...
for(model in unique(d2WEP_retro_all$fitted.through)) {
  cor.N.data = subset(d2WEP_retro_all, fitted.through == model)
...
for(model in unique(d2WEP_retro_all$fitted.through)) {
  cor.WT.data = subset(d2WEP_retro_all, fitted.through == model)
...
levels(obs.pred.pa.cor_all$Metric) = c("Presence-Absence", "Abundance", "Biomass")
  save(obs.pred.pa.cor_all, file = "Analyses/d2WEP_obs_pred_pa_cor.rda")
